...
arr1 = [*range(1, 11, 1)]
arr2 = [*range(1, 11, 1)]
# learning_rates = np.linspace(0.01, 0.3, 25)
# test = [arr1, arr1]
# combinations = list(itertools.product(*test))


# print(combinations)
# combinations = itertools.product(*test[:2])



# print(combinations)

bestArchitecture, MSEs = network_tuner(arr1, arr2)
...
# nn = NeuralNetwork(datasetEncoded, 1, [2], dataRetriever.getPredictionType(), dataRetriever.getDataClass())
# for i in range(maxItter):
#     # We don't call an inital feedforward because backpropagate starts with a feedforward call
#     # batch_size represents the number of data points per batch
#     output = nn._back_propagate(learning_rate=learning_rate, batch_size=batch_size)
...
# # output = nn._feed_forward(testEncoded.drop(dataRetriever.getDataClass(), axis=1), testing=True)
# # print(len(trained))
# final = bestArchitecture['network'].test(testEncoded.drop(dataRetriever.getDataClass(), axis=1))
# output = bestArchitecture['network']._feed_forward(testEncoded.drop(dataRetriever.getDataClass(), axis=1), testing=True)
# actual = testEncoded[dataRetriever.getDataClass()]
